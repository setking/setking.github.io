...
1.  建立连接协议
    ...
    当一端为建立连接而发送它的SYN时，它为连接选择一个初始序号。ISN随时间而变化，因此每个连接都将具有不同的ISN。2. 连接终止协议
    ...
    ![](assets/img/tcp_ip/连接终止期间报文段的正常交换.jpg)
...
1. 第一次超时时间
   ...
   ![](assets/img/tcp_ip/TCP的500ms定时器.jpg)
...
![](assets/img/tcp_ip/TCP半关闭的例子.jpg)
...
![](assets/img/tcp_ip/TCP的状态变迁图.jpg)
...
![](assets/img/tcp_ip/TCP正常连接建立和终止所对应的状态.jpg)
...
![](assets/img/tcp_ip/同时打开期间报文段的交换.jpg)
...
![](assets/img/tcp_ip/同时关闭期间的报文段交换.jpg)
...
3. 限定的远端IP地址
   ...
   ![](assets/img/tcp_ip/TCP服务器本地和远端IP地址及端口号的规范.jpg)
