...
![](assets/img/tcp_ip/SNMP的5种操作.jpg)
...
![](assets/img/tcp_ip/SNMP报文的格式.jpg)
...
![](assets/img/tcp_ip/SNMP报文中的PDU类型.jpg)
...
![](assets/img/tcp_ip/SNMP差错状态的值.jpg)
...
![](assets/img/tcp_ip/管理信息库中的对象标识.jpg)
...
![](assets/img/tcp_ip/UDP组的结构.jpg)

在该组中，包含4个简单变量和1个由两个简单变量组成的表格。下图描述了这4个简单变量：

![](assets/img/tcp_ip/UDP组下的简单变量.jpg)
...
![](assets/img/tcp_ip/udpTable中的变量.jpg)
...
1. system组
   ...
   ![](assets/img/tcp_ip/system组中的简单变量.jpg)

2. interface组

   在本组中只定义了一个简单变量，那就是系统的接口数量，如图所示：

   ![](assets/img/tcp_ip/if组中的简单变量.jpg)

   在该组中，还有一个表格变量，有22列。表格中的每行定义了接口的一些特征参数。如图所示：

   ![](assets/img/tcp_ip/iftable-在接口表中的变量.jpg)
...
3. at组
   ...
   ![](assets/img/tcp_ip/attable-网络地址转换表.jpg)

4. ip组

   ip组定义了很多简单变量和3个表格变量。下图显示了所有的简单变量：

   ![](assets/img/tcp_ip/ip组中的简单变量.jpg)

   ip组中的第一个表格变量是IP地址表。系统的每个IP地址都对应该表格中的一行。每行中包含了5个变量，如图所示：

   ![](assets/img/tcp_ip/ipAddrTable-IP地址表.jpg)

   ip组中的第二个表是IP路由表，如图所示：

   ![](assets/img/tcp_ip/ipRouteTable-IP路由表.jpg)

   ip组的最后一个表是地址转换表，如图所示：

   ![](assets/img/tcp_ip/ipNetToMediaTable-IP地址转换表.jpg)
...
5. icmp组
   ...
   ![](assets/img/tcp_ip/icmp组中的简单变量.jpg)
...
6. tcp组
   ...
   ![](assets/img/tcp_ip/tcp组中的简单变量.jpg)

   tcp组还包括一个表格变量，即TCP连接表，如图所示：

   ![](assets/img/tcp_ip/tcpConnTable-TCP连接表.jpg)
